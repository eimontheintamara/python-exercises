...
class CentralCorridor(Scene):
    ...
    def enter(self):
        print(dedent("""The Gothons of Planet Percal #25 have invaded destroyed your entire crew. You are the last survival member and your last mission is to get the neutrol bomb from the Weapons Armory, put it in the bridge,blow the ship up after getting into an escape pod.You're running down the central corridor the Weapon Armory when a Gothon jumps out, red scaly skin, dark griteeth, and evil clown costume flowing around his hatfilled body. He's blocking the door to the Armory about to pull a weapon to blast you."""))
        action = input("> ")
...
class TheBridge(Scene):
  def enter(self):
     print(dedent("""You burst onto the Bridge with the netron destruct bomb under your arm and surprise 5 Gothons who are trying to take control of the ship. Each of them has an even ugliclown costume than the last. They haven't pulled their weapons out yet, as they see the active bomb under your arm and don't want to set it off. """))
     action = input("> ")
...
class EscapePod(Scene):
   def enter(self):
      print(dedent("""You rush through the ship desperately trying to make it the escape pod before the whole ship explodes. It seems like hardly any Gothons are on the ship, so your run is clear of interference. You get to the chamber with the escape pods, and now need to pick one to take. Some of them could be damaged but you don't have time to look. There's 5 pods, which one do you take?"""))
